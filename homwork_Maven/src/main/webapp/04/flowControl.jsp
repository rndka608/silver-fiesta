<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>04/flowControl.jsp</title>
</head>
<body>
<h4>흐름제어(페이지 이동)</h4>
<pre>
	1. Request Dispatch : 원본 요청에 대한 정보를 페이지를 이동하는 과정에서 전달해주는 방식.
		1) Forward : 페이지를 이동한 이후 최종 도착지에서 응답데이터가 전송
		<%
// 			RequestDispatcher rd =  request.getRequestDispatcher("/05/destination.jsp");
			//requestDespatch 할때 제어해주는 역할로 ()안에는 도착지에대한 정보를 넘겨줌
			//클라이언트에서 사용하나 서버에서 사용하나를 판단하여 절대주소/상대주소 넣기
// 			rd.forward(request, response);
		%>
		2) Include : 페이지를 이동한 후 도착지에서 생성된 결과데이터(버퍼의 내용)을 가지고,
					 출발지(시작페이지)로 다시 돌아오는 구조.
					 최종 응답데이터는 시작페이지에서 전송.
		<%
// 			rd.include(request, response);
			//A에서 만들어진 응답과 B에서 만들어진 응답이 A에서 만들어진것처럼 함께나옴.
		%>
	2. Redirect : 
		 1) 원본 요청이 시작페이지로 전송
		 2) 해당 페이지에서 Body 가 없고,
		 	상태코드(302/307), Location 헤더를 가지고 응답 전송
		 	Stateless 특성에 따라 서버에서는 최초의 요청 정보가 삭제.
		 3) 클라이언트쪽에서 Location 방향으로 새로운 요청이 발생.
		 4) 최종 응답데이터는 3) 번의 새로운 요청에 대한 응답으로 전송.
		 
		 <%
		 	//클라이언트가 사용하는 주소(절대주소)
		 	response.sendRedirect(request.getContextPath() + "/05/destination.jsp");
		    //두가지의 메소드를 사용함. ()안에는 b에대한 정보(도착지)를 입력한다.
		 	//요청은 a에서 했지만 응답데이터는 b에서만 나왔고 주소도 b에대한 주소로 찍힘. 클라이언트에서는 a가아니라 b에서 나온걸 확인할 수 있다.
		 	//첫번째 요청에서는 302 상태코드로 클라이언트에게 돌려주면 클라이언트는 responseHeader를 확인하여 거기에 찍힌 주소로 새롭게 요청한다.
		 %>
</pre>
</body>
</html>