...
<pre>
	** pageContext (PageContext) : JSP 페이지와 관련된 모든 정보를 가진 객체.
	...
	response (HttpServleResponse) : 응답과 관련된 모든 정보를 캡슐화한 객체
	out (JSPWriter) : 출력버프에 데이터를 기록하거나 버퍼를 제어하기 위해 사용되는 출력 스트림.
	...
	exception(Throwable) : 발생한 예외에 대한 정보르 가진 객체
			: 예외(에러)가 발생한 경우, 예러를 처리하는 페이지에서 사용됨.
